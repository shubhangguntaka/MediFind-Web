...
import { reverseGeocodeCoordinates } from '../services/apiClient';
...
const MapSelector: React.FC<MapSelectorProps> = ({ onClose, onLocationSelect }) => {
  ...
  useEffect(() => {
    if (mapContainerRef.current && !mapInstanceRef.current) {
      const map = L.map(mapContainerRef.current, { center: DEFAULT_CENTER, zoom: DEFAULT_ZOOM, zoomControl: false });
  ...
  const handleGoToMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => setLocationError(`Could not get your location: ${error.message}`)
    );
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm" aria-modal="true" role="dialog">
      <div className="relative w-full max-w-2xl h-[90vh] max-h-[700px] p-4 sm:p-6 m-4 bg-white dark:bg-slate-800 rounded-2xl shadow-xl flex flex-col">
        ...
        <div className="flex-grow rounded-lg overflow-hidden relative bg-slate-200 dark:bg-slate-700">
          ...
          <button type="button" onClick={handleGoToMyLocation} className="absolute bottom-4 left-4 z-[1000] w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-700 rounded-full shadow-lg text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-primary-500" title="Go to my location">
        ...
        <div className="mt-4 flex flex-col sm:flex-row gap-3">
          <button type="button" onClick={onClose} className="w-full sm:w-auto px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600">Cancel</button>
          <button type="button" onClick={handleConfirm} disabled={isProcessing} className="flex-grow flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-primary-400 disabled:cursor-wait">
