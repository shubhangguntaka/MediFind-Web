...
import { analyzeMedicineImage } from '../services/apiClient';
...
const SmartScanModal: React.FC<SmartScanModalProps> = ({ onClose, onMedicinesFound }) => {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      if (result?.error) { showToast(result.error, 'error'); setStep('upload'); return; }
  ...
  const renderModeSelection = () => (
    <div className="space-y-4 animate-fade-in-down">
      ...
      <div className="grid gap-3">
        <button onClick={() => handleModeSelect('prescription')} className="w-full flex items-center gap-4 p-5 bg-white dark:bg-slate-700 border-2 border-gray-100 dark:border-slate-600 rounded-2xl hover:border-primary-500 dark:hover:border-primary-500 transition-all group shadow-sm hover:shadow-md">
        ...
        <button onClick={() => handleModeSelect('identification')} className="w-full flex items-center gap-4 p-5 bg-white dark:bg-slate-700 border-2 border-gray-100 dark:border-slate-600 rounded-2xl hover:border-primary-500 dark:hover:border-primary-500 transition-all group shadow-sm hover:shadow-md">
  ...
  const renderUpload = () => (
    <div className="space-y-6 text-center animate-fade-in-down">
      ...
      <div className={`w-full aspect-[4/3] border-3 border-dashed rounded-3xl flex flex-col items-center justify-center p-6 relative overflow-hidden transition-all duration-300 ${imagePreview ? 'border-primary-500 ring-4 ring-primary-500/10' : 'border-gray-200 dark:border-slate-600 hover:border-primary-300'}`}>
        {imagePreview ? (
          <img src={imagePreview} alt="Preview" className="w-full h-full object-cover rounded-2xl" />
        ) : (
          <div className="text-gray-400 flex flex-col items-center gap-4">
            <div className="p-6 bg-gray-50 dark:bg-slate-700 rounded-full shadow-inner"><CameraIcon className="w-16 h-16" /></div>
        ...
        <input type="file" accept="image/*" capture="environment" ref={fileInputRef} onChange={handleFileSelect} className="absolute inset-0 opacity-0 cursor-pointer" />
      </div>
      <div className="flex gap-3">
        <button onClick={() => setImagePreview(null)} className="flex-1 py-4 px-6 bg-gray-100 dark:bg-slate-700 dark:text-white rounded-2xl font-bold hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50" disabled={!imagePreview}>Retake</button>
        <button onClick={startAnalysis} className="flex-1 py-4 px-6 bg-primary-600 text-white rounded-2xl font-bold shadow-lg hover:bg-primary-700 hover:shadow-primary-500/20 transition-all disabled:bg-primary-300" disabled={!imagePreview}>Start AI Analysis</button>
      </div>
    </div>
  );
      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-fade-in-down border border-white/10">
        <div className="p-8 sm:p-10">
          <div className="flex justify-end mb-2">
            <button onClick={onClose} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-all"><CloseIcon className="w-7 h-7" /></button>
          </div>
