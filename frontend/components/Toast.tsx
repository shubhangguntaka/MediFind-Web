...
const Toast: React.FC = () => {
  ...
  return (
    <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm sm:max-w-md animate-toast-in" role="alert">
      <div className={`relative flex items-start gap-4 p-4 rounded-lg shadow-lg border ${bgColors[toast.type]}`}>
        <div className="flex-shrink-0">{icons[toast.type]}</div>
        <div className={`flex-grow text-sm font-semibold ${textColors[toast.type]}`}>{toast.message}</div>
        <button onClick={hideToast} className="flex-shrink-0 -mt-1 -mr-1 p-1 rounded-full hover:bg-black/10" aria-label="Dismiss"><CloseIcon className={`w-5 h-5 ${textColors[toast.type]} opacity-70 hover:opacity-100`} /></button>
      </div>
    </div>
  );
};
