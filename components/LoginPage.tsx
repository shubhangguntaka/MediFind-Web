...
const LoginPage: React.FC<LoginPageProps> = ({ onClose, initialMode }) => {
  ...
  // OTP step removed — Supabase uses email link recovery (see handleForgotEmailSubmit)
  ...
  const renderForgotEmail = () => (
    <form onSubmit={handleForgotEmailSubmit} className="space-y-4">
      <p className="text-sm text-center text-gray-600 dark:text-gray-400">Enter your email address and we'll send you a password reset link.</p>
      <fieldset disabled={isProcessing} className="space-y-4">
        <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-slate-700 dark:border-slate-600 dark:text-white" placeholder="Email Address" aria-label="Email Address" />
        <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400 disabled:cursor-wait">
  ...
  const renderForgotSent = () => (
    <div className="space-y-4 text-center">
      <p className="text-sm text-gray-600 dark:text-gray-400">
        ...
        <span className="font-semibold text-gray-800 dark:text-gray-200">{resetEmail}</span>.
      </p>
      <p className="text-sm text-gray-500 dark:text-gray-400">
        Click the link in the email to set a new password. The link expires in 1 hour.
      </p>
      <button onClick={() => handleSetView('signin')} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">
        Back to Sign In
      </button>
    </div>
  );
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'forgot_sent': return renderForgotSent();
