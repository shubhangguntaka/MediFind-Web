        ...
        {!hasSearched && (
          <div className="max-w-6xl mb-8 animate-fade-in-down px-2">
            <div className="flex flex-wrap gap-2 items-center justify-center">
              <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest flex items-center gap-2 mr-2">
                <SearchIcon className="w-4 h-4" /> Popular:
              </span>
              {SUGGESTED_MEDICINES.map(med => (
                <button
                  key={med}
                  onClick={() => handleSearch(med)}
                  className="px-4 py-2 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/30 hover:text-primary-700 transition-all border border-gray-100 dark:border-slate-700 hover:border-primary-200 dark:hover:border-primary-800 shadow-sm"
                >
                  {med}
                </button>
              ))}
